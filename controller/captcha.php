<?php
$digit       = 4; //Verification code digits
$digit_width = 55; // Unit verification code width
$height      = 100; // Captcha Height
$letter_list = str_split("123456789"); //Karakter default, 0 mudah dikacaukan dengan O, jadi tidak perlu, huruf dan simbol juga didukung

$width       = $digit_width * $digit;

$code_list   = array(); // Array karakter captcha
$block_list  = array(); // susunan blok warna latar belakang
for ($i = 0; $i < $digit; $i++) {
	$code_list[$i]          = $letter_list[mt_rand(0, count($letter_list) - 1)];
	$block_list[$i * 2]     = $i * 2;
	$block_list[$i * 2 + 1] = $i * 2 + 1;
}
$verification_code = implode('', $code_list); // kode verifikasi akhir
// for debug only
...
// Fill the background square to prevent simple grayscale based binarization algorithm
...
foreach ($block_list as $offset => $code) {
	...
	//draw a background color block
	imagefilledrectangle($background, $digit_width * $offset / 2, 0, $digit_width * ($offset / 2 + 0.5), $height, $background_color[$offset]);
}

// Fill in the verification code and add random font size and rotation and displacement to achieve high probability of overlapping characters and prevent them from being easily cut
foreach ($code_list as $offset => $code) {
	...
	imagettftext(
		...
		mt_rand(-25, 25), // twist angle
		$digit_width * $offset + mt_rand(0, $digit_width * 0.5), //x-axis position
		mt_rand($font_height * 0.9, $font_height * 1.1), //y-axis position
		$font_color, //color
		LibraryPath . 'fonts/SourceCodePro-Black.ttf', //font
		$code //character
	);
}

// Fill noise and lines
$noise_number = 0.3 * $digit_width * $height / 2; //Noise ratio
$line_number  = min(0.1 * $height, 3); 
$line_width   = 0.03 * $height; 
